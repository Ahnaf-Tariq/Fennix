...
function SegmentLabel({
  ...
  const labelEl = showLabels && (
    <span className="max-w-[6.5rem] text-center text-[11px] font-semibold leading-tight text-white/90 sm:text-xs">
      {stage.label}
    </span>
  );
...
export function FunnelChart({
  ...
  const [internalHoveredIndex, setInternalHoveredIndex] = useState<
    number | null
  >(null);
  const [pointerPos, setPointerPos] = useState({ x: 0.5, y: 0.42 });
  ...
  function handleSegmentPointerMove(event: MouseEvent<HTMLDivElement>) {
    const rect = event.currentTarget.getBoundingClientRect();
    if (rect.width <= 0 || rect.height <= 0) return;
    setPointerPos({
      x: Math.min(
        0.88,
        Math.max(0.12, (event.clientX - rect.left) / rect.width),
      ),
      y: Math.min(
        0.82,
        Math.max(0.18, (event.clientY - rect.top) / rect.height),
      ),
    });
  }
          ...
          {data.map((stage, i) => {
            ...
            return (
              <motion.div
                ...
                onMouseMove={handleSegmentPointerMove}
                onMouseLeave={() => setHoveredIndex(null)}
                style={{ ...posStyle, zIndex: isHovered ? 40 : 20 }}
              ...
              >
                ...
                <AnimatePresence>
                  {isHovered && stage.description && (
                    <div
                      className="pointer-events-none absolute z-50 w-[min(15.5rem,calc(100vw-1.5rem))] sm:w-[min(17.5rem,78vw)]"
                      style={
                        horiz
                          ? {
                        ...
                        <div className="relative overflow-hidden rounded-2xl border border-zinc-200/80 bg-white shadow-[0_18px_40px_-18px_rgba(6,27,49,0.28),0_0_0_1px_rgba(20,86,168,0.04)]">
                          ...
                          <div className="p-3 sm:p-5">
                            <div className="mb-2 flex items-center gap-2.5 sm:mb-3 sm:gap-3">
                              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[linear-gradient(145deg,#1456a8_0%,#0c4a8c_55%,#061b31_100%)] text-[10px] font-semibold tracking-wide text-white shadow-sm sm:h-9 sm:w-9 sm:text-[11px]">
